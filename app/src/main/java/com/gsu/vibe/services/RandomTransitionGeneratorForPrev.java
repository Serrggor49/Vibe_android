...
public class RandomTransitionGeneratorForPrev implements TransitionGenerator {
    ...
    public RandomTransitionGeneratorForPrev() {
        this(DEFAULT_TRANSITION_DURATION, new AccelerateDecelerateInterpolator());
    }


    public RandomTransitionGeneratorForPrev(long transitionDuration, Interpolator transitionInterpolator) {
    ...
    private RectF generateRandomRect(RectF drawableBounds, RectF viewportRect) {
        ...
        // эта функия возвращает значение 0.1...1. Значечние 0.1 - максимальный зум.
        // Становятся видны пикселика картинки. Чтоб такой фигни не б

        float randomFloat = MathUtils.truncate(mRandom.nextFloat(), 2);
        //float factor = MIN_RECT_FACTOR + ((1 - MIN_RECT_FACTOR) * randomFloat); //функция по умолчанию
        //float factor = MIN_RECT_FACTOR + ((1 - MIN_RECT_FACTOR) * 0.01f); // максимальный зум
        //float factor = MIN_RECT_FACTOR + ((1 - MIN_RECT_FACTOR) * 0.05f); // нулевой зум

        Random r = new Random();
        float factor = 0.85f + r.nextFloat() * (1f - 0.85f);  // рандом от 0.85...1
        Log.d("MyLogs552", "randomFloat = " + factor);
